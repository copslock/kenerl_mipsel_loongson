Received: with ECARTIS (v1.0.0; list linux-mips); Tue, 01 Mar 2011 17:12:35 +0100 (CET)
Received: from nbd.name ([46.4.11.11]:51010 "EHLO nbd.name"
        rhost-flags-OK-OK-OK-OK) by eddie.linux-mips.org with ESMTP
        id S1491822Ab1CAQM2 (ORCPT <rfc822;linux-mips@linux-mips.org>);
...
Subject: [PATCH V2 02/10] MIPS: lantiq: add SoC specific code for XWAY family
Date:   Tue,  1 Mar 2011 17:13:18 +0100
Message-Id: <1298996006-15960-3-git-send-email-blogic@openwrt.org>
...
+		printk(KERN_ERR "failed to setup lantiq gpio: %s\n", name);
+		return -EBUSY;
+	}
+	if (dir)
...
+
+static __iomem void *ltq_pmu_membase;
+
+void
...
+	if (!ltq_pmu_membase) {
+		printk(KERN_ERR "Failed to remap pmu memory\n");
+		unreachable();
+	}
+	return 0;
+}
...
+static __iomem void *ltq_rcu_membase;
+
+static void
...
+	if (!ltq_rcu_membase) {
+		printk(KERN_ERR "Failed to remap rcu memory\n");
+		unreachable();
+	}
+
