Received: with ECARTIS (v1.0.0; list linux-mips); Mon, 12 Jan 2015 15:11:18 +0100 (CET)
Received: from mail-wi0-f178.google.com ([209.85.212.178]:52639 "EHLO
        mail-wi0-f178.google.com" rhost-flags-OK-OK-OK-OK)
        by eddie.linux-mips.org with ESMTP id S27011228AbbALOLOjMzMW (ORCPT
        <rfc822;linux-mips@linux-mips.org>); Mon, 12 Jan 2015 15:11:14 +0100
Received: by mail-wi0-f178.google.com with SMTP id em10so14782244wid.5;
        Mon, 12 Jan 2015 06:11:09 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20120113;
        h=sender:from:to:cc:subject:date:message-id:in-reply-to:references;
        bh=5P82vXsYKNa7mdy7+2ROS8e6gGc7flPvQK8K5+8KxOk=;
        b=
X-Received: by 10.194.23.6 with SMTP id i6mr57550901wjf.26.1421071869348;
        Mon, 12 Jan 2015 06:11:09 -0800 (PST)
Received: from localhost.localdomain (37-48-43-149.tmcz.cz. [37.48.43.149])
        by mx.google.com with ESMTPSA id n3sm10376351wiw.5.2015.01.12.06.11.05
        (version=TLSv1.2 cipher=ECDHE-RSA-AES128-GCM-SHA256 bits=128/128);
        Mon, 12 Jan 2015 06:11:07 -0800 (PST)
...
Date:   Mon, 12 Jan 2015 15:10:05 +0100
Message-Id: <1421071809-17402-3-git-send-email-tomeu.vizoso@collabora.com>
...
--- a/drivers/clk/clk.c
+++ b/drivers/clk/clk.c
@@ -743,6 +743,8 @@ struct clk *__clk_lookup(const char *name)
...
@@ -1438,8 +1440,11 @@ static struct clk_core *clk_calc_new_rates(struct clk_core *clk,
...
@@ -1598,30 +1615,10 @@ static void clk_change_rate(struct clk_core *clk)
...
+	unsigned long rate = req_rate;
 	...
 	if (!clk)
@@ -1631,17 +1628,17 @@ int clk_set_rate(struct clk *clk, unsigned long rate)
 	clk_prepare_lock();
...
@@ -2138,6 +2214,10 @@ struct clk *__clk_register(struct device *dev, struct clk_hw *hw)
 	else
 		clk->owner = NULL;
...
+
 	...
 	if (ret)
 		return ERR_PTR(ret);
@@ -2146,6 +2226,19 @@ struct clk *__clk_register(struct device *dev, struct clk_hw *hw)
 }
...
@@ -2205,12 +2298,14 @@ struct clk *clk_register(struct device *dev, struct clk_hw *hw)
 		}
 	}
...
@@ -2416,7 +2511,7 @@ int __clk_get(struct clk *clk)
...
@@ -2543,6 +2638,11 @@ struct clk *__clk_create_clk(struct clk_hw *hw, const char *dev_id,
 	clk->core = hw->core;
 	clk->dev_id = dev_id;
 	clk->con_id = con_id;
