Received: with ECARTIS (v1.0.0; list linux-mips); Wed, 25 Mar 2009 16:53:57 +0000 (GMT)
Received: from mail-fx0-f175.google.com ([209.85.220.175]:10414 "EHLO
	...
	id S28575201AbZCYQtn (ORCPT <rfc822;linux-mips@linux-mips.org>);
	Wed, 25 Mar 2009 16:49:43 +0000
Received: by mail-fx0-f175.google.com with SMTP id 23so148335fxm.0
        for <linux-mips@linux-mips.org>; Wed, 25 Mar 2009 09:49:43 -0700 (PDT)
DKIM-Signature:	v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=googlemail.com; s=gamma;
        h=domainkey-signature:received:received:sender:from:to:cc:subject
         :date:message-id:x-mailer:in-reply-to:references;
        bh=2bvEXqg7tUR9h2m0XH2oa1u1sB6u+jKr07puTUQ3U8A=;
        b=qQp/x2FPny9cwk0noQYKUX53b6neGhPFQWtHJJgf6RMotlK0YVRJKHJm4jx/PnLDa5
         bl2WPtOnC18fcpIWj0wvO05P7W/+225aqrsT54z4kG3MsRMeX6qxLBpuAAwxkCMBHP6+
         15PLc5Qx8DjwLvVwr/SOlGYRc+YRE66G+JToc=
DomainKey-Signature: a=rsa-sha1; c=nofws;
        d=googlemail.com; s=gamma;
        h=sender:from:to:cc:subject:date:message-id:x-mailer:in-reply-to
         :references;
        b=BLdiCMwaiZqkU/04FWQpOtUqdg3McrqdzCTn1Nqp+/9nLXIJ3eMY5Dipn+GTsQp0WV
         OcuswDqDlz10iOXdigAkM1BBxBHdUUNMicZeChesRAXskmnS86x28NILx4vMIBMSwhcf
         jNn0RcOpv7/ZvtT+QPb27ZrFHE6k17/UDyvpE=
Received: by 10.103.220.18 with SMTP id x18mr4264953muq.135.1237999782777;
        Wed, 25 Mar 2009 09:49:42 -0700 (PDT)
Received: from localhost.localdomain (p5496CCD7.dip.t-dialin.net [84.150.204.215])
        by mx.google.com with ESMTPS id e10sm14966093muf.41.2009.03.25.09.49.40
        (version=TLSv1/SSLv3 cipher=RC4-MD5);
        Wed, 25 Mar 2009 09:49:42 -0700 (PDT)
From:	Manuel Lauss <mano@roarinelk.homelinux.net>
To:	Linux-MIPS <linux-mips@linux-mips.org>
Cc:	Manuel Lauss <mano@roarinelk.homelinux.net>
Subject: [PATCH 6/6] Alchemy: extended DB1200 board support.
Date:	Wed, 25 Mar 2009 17:49:33 +0100
Message-Id: <1237999773-5174-7-git-send-email-mano@roarinelk.homelinux.net>
...
References: <1237999773-5174-1-git-send-email-mano@roarinelk.homelinux.net>
 ...
 <1237999773-5174-6-git-send-email-mano@roarinelk.homelinux.net>
Return-Path: <manuel.lauss@googlemail.com>
X-Envelope-To: <"|/home/ecartis/ecartis -s linux-mips"> (uid 0)
X-Orcpt: rfc822;linux-mips@linux-mips.org
Original-Recipient: rfc822;linux-mips@linux-mips.org
X-archive-position: 22148
...
---
 ...
 arch/mips/alchemy/devboards/Makefile             |    7 +-
 ...
 14 files changed, 1893 insertions(+), 830 deletions(-)
...
index 5c76c64..29e1db9 100644
--- a/arch/mips/alchemy/common/platform.c
+++ b/arch/mips/alchemy/common/platform.c
@@ -270,7 +270,6 @@ static struct platform_device au1200_mmc0_device = {
...
-#ifndef CONFIG_MIPS_DB1200
 static struct resource au1200_mmc1_resources[] = {
 	[0] = {
 		.start          = SD1_PHYS_ADDR,
@@ -305,7 +304,6 @@ static struct platform_device au1200_mmc1_device = {
...
@@ -344,10 +342,8 @@ static struct platform_device *au1xxx_platform_devices[] __initdata = {
...
+obj-y += prom.o
+ifeq ($(CONFIG_MIPS_DB1200),n)
+	obj-y += ../common/platform.o
+endif
 obj-$(CONFIG_PM)		+= pm.o
 obj-$(CONFIG_MIPS_PB1000)	+= pb1000/
 obj-$(CONFIG_MIPS_PB1100)	+= pb1100/
@@ -11,7 +14,7 @@ obj-$(CONFIG_MIPS_PB1500)	+= pb1500/
...
+	},
+	{
+		/* OmniVision OV9640 camera */
+		I2C_BOARD_INFO("ov9640", 0x30),
+	},
+	/* WM8731 audio codec is registered by audio code */
+};
+
+/**********************************************************************/
+
...
+static struct resource au1200_ohci_res[] = {
+	[0] = {
+		.start	= USB_OHCI_BASE,
...
+
+static struct platform_device au1200_ohci_dev = {
+	.name		= "au1xxx-ohci",
+	.id		= 0,
+	.dev = {
...
+		.coherent_dma_mask	= DMA_32BIT_MASK,
+	},
+	.num_resources	= ARRAY_SIZE(au1200_ohci_res),
+	.resource	= au1200_ohci_res,
+};
+
+static struct resource au1200_ehci_res[] = {
+	[0] = {
+		.start	= USB_EHCI_BASE,
...
+
+static struct platform_device au1200_ehci_dev = {
+	.name		= "au1xxx-ehci",
+	.id		= 0,
+	.dev = {
...
+		.coherent_dma_mask	= DMA_32BIT_MASK,
+	},
+	.num_resources	= ARRAY_SIZE(au1200_ehci_res),
+	.resource	= au1200_ehci_res,
+};
+
+/* Au1200 UDC (USB gadget controller) */
+static struct resource au1200_gdt_res[] = {
+	[0] = {
+		.start	= USB_UDC_BASE,
...
+
+static struct platform_device au1200_gdt_dev = {
+	.name		= "au1xxx-udc",
+	.id		= 0,
+	.dev = {
...
+		.coherent_dma_mask	= DMA_32BIT_MASK,
+	},
+	.num_resources	= ARRAY_SIZE(au1200_gdt_res),
+	.resource	= au1200_gdt_res,
+};
+
+/* Au1200 UOC (USB OTG controller) */
+static struct resource au1200_otg_res[] = {
+	[0] = {
+		.start	= USB_UOC_BASE,
...
+
+static struct platform_device au1200_otg_dev = {
+	.name		= "au1xxx-uoc",
+	.id		= 0,
+	.dev = {
...
+		.coherent_dma_mask	= DMA_32BIT_MASK,
+	},
+	.num_resources	= ARRAY_SIZE(au1200_otg_res),
+	.resource	= au1200_otg_res,
+};
+
+/**********************************************************************/
+
...
+/**********************************************************************/
+
+static void alchemy_uart_pm(struct uart_port *port, unsigned int state,
+			   unsigned int old_state)
+{
+	if (state == 0) {		/* power on */
+		au_writel(0, (unsigned long)port->membase + UART_MOD_CNTRL);
+		au_sync();
+		au_writel(1, (unsigned long)port->membase + UART_MOD_CNTRL);
+		au_sync();
+		au_writel(3, (unsigned long)port->membase + UART_MOD_CNTRL);
+		au_sync();
...
+}
+
+static struct plat_serial8250_port db1200_uart_data[] = {
+	{
+		.iobase		= UART0_ADDR,
+		.membase	= (void __iomem *)UART0_ADDR,
+		.mapbase	= CPHYSADDR(UART0_ADDR),
+		.irq		= AU1200_UART0_INT,
+		.regshift	= 2,
+		.iotype		= UPIO_AU,
+		.flags		= UPF_SKIP_TEST,
+		.pm		= alchemy_uart_pm,
...
+	{
+		.iobase		= UART1_ADDR,
+		.membase	= (void __iomem *)UART1_ADDR,
+		.mapbase	= CPHYSADDR(UART1_ADDR),
+		.irq		= AU1200_UART1_INT,
+		.regshift	= 2,
+		.iotype		= UPIO_AU,
+		.flags		= UPF_SKIP_TEST,
+		.pm		= alchemy_uart_pm,
...
+	.id		= 0,
+	.dev = {
+		.platform_data = db1200_uart_data,
+	},
+};
+
+/**********************************************************************/
...
+
+static struct platform_device mmc_dev = {
+	.name		= "au1xxx-mmc",
+	.id		= 0,
+	.dev = {
...
+};
+
+static struct platform_device pcmcia_dev = {
+	.name 		= "au1x00-pcmcia",
+	.id 		= 0,
+};
+
+static struct platform_device *db1200_devs[] __initdata = {
+	NULL,		/* PSC0, decided by S6.8 */
+	&uart_dev,
+	&smc91x_dev,
+	&mmc_dev,
+	&rtc_dev,
...
+
+	/* fill in uartclk */
+	for (i = 0; db1200_uart_data[i].flags; i++)
+		db1200_uart_data[i].uartclk = uartclk;
+
...
+
+/* DB1200 addon: use the hexleds to count the number of times
+ * the cpu has entered wait.  Use the dots to indicate whether
+ * the system is currently idle or active (dots off = sleep,
+ * dots on = working) for cases where the number doesn't change
+ * for a long(er) period of time.		-- mlau
+ */
...
+{
+	bcsr->resets = 0;
+	bcsr->system = 0; /* reset bit is low active! */
+}
+
...
+	au_sync();
+
+	/* Clock configurations: PSC0 50MHz, derived from CPU clock.
+	 * all others off.
+	 */
...
+	au_sync();
+
+	/* psc0_intclk comes from  FREQGen0 directly */
...
@@ -86,6 +100,7 @@ CONFIG_CPU_MIPS32_R1=y
...
@@ -98,6 +113,7 @@ CONFIG_CPU_MIPS32=y
 CONFIG_CPU_MIPSR1=y
...
+CONFIG_HARDWARE_WATCHPOINTS=y
 ...
 #
 # Kernel type
@@ -112,7 +128,6 @@ CONFIG_CPU_HAS_PREFETCH=y
...
@@ -120,168 +135,182 @@ CONFIG_GENERIC_HARDIRQS=y
...
@@ -292,107 +321,24 @@ CONFIG_IP_FIB_HASH=y
...
@@ -402,21 +348,21 @@ CONFIG_NF_CONNTRACK_PROC_COMPAT=y
...
+# CONFIG_NET_9P is not set
 ...
 #
 # Device Drivers
@@ -425,25 +371,23 @@ CONFIG_NET_CLS_ROUTE=y
 #
...
@@ -456,6 +400,7 @@ CONFIG_MTD_BLOCK=y
...
@@ -481,7 +426,6 @@ CONFIG_MTD_CFI_UTIL=y
...
@@ -494,6 +438,9 @@ CONFIG_MTD_ALCHEMY=y
 #
...
@@ -505,224 +452,110 @@ CONFIG_MTD_ALCHEMY=y
...
+# CONFIG_INPUT_POLLDEV is not set
 ...
 #
 # Userland interfaces
@@ -739,7 +573,6 @@ CONFIG_INPUT_MOUSEDEV_PSAUX=y
...
@@ -749,28 +582,33 @@ CONFIG_INPUT_EVDEV=y
...
-# CONFIG_AU1X00_GPIO is not set
 ...
 #
 # Serial drivers
@@ -778,8 +616,8 @@ CONFIG_VT_HW_CONSOLE_BINDING=y
...
@@ -789,22 +627,9 @@ CONFIG_SERIAL_8250_AU1X00=y
...
-# CONFIG_DTLK is not set
 ...
 #
@@ -813,223 +638,528 @@ CONFIG_LEGACY_PTY_COUNT=256
...
@@ -1040,19 +1170,18 @@ CONFIG_FAT_DEFAULT_IOCHARSET="iso8859-1"
 CONFIG_PROC_FS=y
 CONFIG_PROC_KCORE=y
...
@@ -1061,187 +1190,192 @@ CONFIG_CONFIGFS_FS=m
 CONFIG_JFFS2_FS=y
...
+CONFIG_CMDLINE="root=/dev/hda3 rootfstype=ext2 console=tty video=au1200fb:panel:bs"
...
+};
+
+static struct wm8731_setup_data db1200_i2s_data = {
+	.i2c_bus	= 0,
+	.i2c_address	= 0x1b,
+};
+
