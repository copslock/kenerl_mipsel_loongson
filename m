Received: with ECARTIS (v1.0.0; list linux-mips); Tue, 21 Aug 2012 03:55:49 +0200 (CEST)
Received: from [69.28.251.93] ([69.28.251.93]:46253 "EHLO b32.net"
        rhost-flags-FAIL-FAIL-OK-OK) by eddie.linux-mips.org with ESMTP
        id S1903611Ab2HUBzi (ORCPT <rfc822;linux-mips@linux-mips.org>);
        Tue, 21 Aug 2012 03:55:38 +0200
Received: (qmail 15601 invoked from network); 21 Aug 2012 01:55:33 -0000
Received: from unknown (HELO vps-1001064-677.cp.jvds.com) (127.0.0.1)
  by 127.0.0.1 with (DHE-RSA-AES128-SHA encrypted) SMTP; 21 Aug 2012 01:55:33 -0000
Received: by vps-1001064-677.cp.jvds.com (sSMTP sendmail emulation); Mon, 20 Aug 2012 18:55:32 -0700
From:   Kevin Cernekee <cernekee@gmail.com>
To:     balbi@ti.com, ralf@linux-mips.org, sebastian@breakpoint.cc
Cc:     linux-mips@linux-mips.org, linux-usb@vger.kernel.org
Subject: [PATCH V2] usb: gadget: bcm63xx UDC driver
Date:   Mon, 20 Aug 2012 18:49:36 -0700
Message-Id: <b3bb6f2afb3ed82fd1e64563c68fb8df@localhost>
User-Agent: vim 7.2
MIME-Version: 1.0
Content-Type: text/plain; charset=us-ascii
Content-Disposition: inline
X-archive-position: 34298
...
---
 ...
 drivers/usb/gadget/bcm63xx_udc.c  | 2442 +++++++++++++++++++++++++++++++++++++
 drivers/usb/gadget/gadget_chips.h |    3 +
 4 files changed, 2459 insertions(+)
 create mode 100644 drivers/usb/gadget/bcm63xx_udc.c


V2 changes:

Rename all non-trivial symbols to bcm63xx_* or iudma_*
Move most HW init/deinit into udc_start/udc_stop calls
Remove debug prints from udc_start/udc_stop calls
Halt the USBD clocks when stopped
Fix duplicate memory allocation in iudma_uninit()
Don't use a workqueue for ep0 handling
Remove -ERANGE check in bcm63xx_ep_enable()
Don't use DMA_ADDR_INVALID
Clean up IUDMA data ISR multiplexing

Quick sanity tests still pass; I will run more tests overnight.

Checkpatch warnings are only for seq_printf() long lines (and turn into
"split string" warnings if I split the strings).

Do you think I should add support for the new ->reset callback now?
...
+ * @ep0_irqflags: Temp storage to allow dropping spinlock for SETUP callbacks.
...
+ * @ep0_worker_active: Prevent re-entering worker loop from a setup callback.
...
+static void bcm63xx_ep0_process(struct bcm63xx_udc *udc);
+
+/***********************************************************************
...
+	int rc;
+
+	spin_unlock_irqrestore(&udc->lock, udc->ep0_irqflags);
+	rc = udc->driver->setup(&udc->gadget, ctrl);
+	spin_lock_irqsave(&udc->lock, udc->ep0_irqflags);
+	return rc;
+}
+
+/**
...
+ */
+static void bcm63xx_ep0_spoof_set_cfg(struct bcm63xx_udc *udc)
+{
+	struct usb_ctrlrequest ctrl;
+
...
+	ctrl.wIndex = 0;
+	ctrl.wLength = 0;
+	if (bcm63xx_ep0_setup_callback(udc, &ctrl) < 0) {
...
+ * @udc: Reference to the device controller.
+ */
+static void bcm63xx_ep0_spoof_set_iface(struct bcm63xx_udc *udc)
+{
+	struct usb_ctrlrequest ctrl;
+
...
+{
+	spin_lock_irqsave(&udc->lock, udc->ep0_irqflags);
+	if (!udc->ep0_worker_active) {
+		udc->ep0_worker_active = 1;
+		while (bcm63xx_ep0_one_round(udc) == 0)
+			;
+		udc->ep0_worker_active = 0;
+	}
+	spin_unlock_irqrestore(&udc->lock, udc->ep0_irqflags);
+}
+
...
+	u32 stat;
+	bool disconnected = false, process_ep0 = false;
+
...
+	int rc;
+	bool is_done = false, process_ep0 = false;
+
+	spin_lock(&udc->lock);
+
+	usb_dmac_writel(udc, ENETDMAC_IR_BUFDONE_MASK,
